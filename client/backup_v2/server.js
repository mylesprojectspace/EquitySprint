...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', ({ roomId, playerName }) => {
    ...
    io.to(roomId).emit('lobby-update', {
      players: room.playerNames.map((n, i) => ({ name: n, connected: !!room.playerSlots[i] })),
  ...
  socket.on('add-bot', () => {
    ...
    io.to(roomId).emit('lobby-update', {
      players:    room.playerNames.map((n, i) => ({ name: n, connected: !!room.playerSlots[i], isBot: room.botSlots[i] })),
      maxPlayers: room.maxPlayers,
    });
  ...
  socket.on('start-game', () => {
    ...
    room.G       = engine.initGame(room.playerNames, room.botSlots);
    room.started = true;
