  ...
  el.innerHTML = `
    ...
    <div class="sidebar-summary-row">
      <div class="sidebar-summary-box">
        <div class="ssb-label">Rent/yr</div>
        <div class="ssb-value">${fmt(p.rentalIncome || 0)}</div>
      </div>
      <div class="sidebar-summary-box">
        <div class="ssb-label">Prop Value</div>
        <div class="ssb-value">${fmt((p.properties || []).reduce((s, pr) => s + (pr.currentValue || 0), 0))}</div>
      </div>
    </div>
  `;
...
// ── Right Panels (Events cards + Chat) ──────────────────────
...
function renderRightPanels() {
  const el = document.getElementById('right-panels');
  if (!el || !G) return;

  // Top panel: active events as cards
  let eventsHtml = '';
  const r = G.activeRestrictions;
  ...
  // Bottom panel: chat (rendered separately so messages don't reset on re-render)
  el.innerHTML = `
    <div class="right-panel">
      <div class="right-panel-title">⚡ Active Events</div>
      <div class="right-panel-body rp-events-body">${eventsHtml}</div>
    </div>
    <div class="right-panel" id="chat-panel">
      <div class="right-panel-title">💬 Chat</div>
      <div class="right-panel-body" id="chat-messages"></div>
...
function renderBottomStrip() {
  ...
  el.innerHTML = `<div id="strip-right-panel">${rightHtml}</div>` +
